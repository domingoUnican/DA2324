...
(define (seval exp environ)
  ...
  (cond ((primitiva? exp) exp)                            ; Primitive just "are". Return back
        ...
        ((quote? exp) exp)
        ((lambda? exp) (genera-lambda exp environ))
        ; ((cond? exp) ...)
        ; ((let ...))
        ; ((delay...))
        ((begin? exp) (begin-expressions exp))
        ((aplicacion-procedimiento? exp) (aplica-procedimiento exp environ))
...
(define (aplica-procedimiento exp env)
  (displayln exp)
...
(define (genera-funcion exp environ)
  (lambda (x . z) (begin
                  (seval (car (cdr (cdr exp))) (crear_nuevo_ambiente_funcion (car (cdr exp)) (cons x z) environ)))))
...
(define (genera-lambda exp environ)
  (lambda (arg1 . arg2) (begin
               (let
                   ((new_env (crear_nuevo_ambiente_funcion (car (cdr exp)) (append (list arg1) arg2) environ)))
                 (seval (car (cdr (cdr exp))) new_env)
                 )
               )))
...
(anade-ambiente environ '(+ - * < > = / true false) (list + - * < > = / true false))
